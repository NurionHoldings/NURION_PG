...
- 독립 migration 작업이 API 이미지에서 실행 가능하도록 스크립트를 이미지에 포함한다. Sol 검토에서 누락을 발견했다.
- 운영 배포 절차와 외부 차단조건은 `deploy/PRODUCTION_RELEASE.md`에 기록한다.

## 검증

- `test_api_runtime.py`: 12 PASS.
- `test_ops_deployment_readiness.py`: 5 PASS. 실제 lifespan 모의 연결, schema drift 및 연결 오류, Docker 이미지에 migration 명령 포함 여부를 검증한다.
...
- PR #436의 최초 HEAD `f26222c`에서 CI 1회가 시작되었으나, Sol 검토로 Dockerfile 누락을 발견하여 수정한다. 최종 HEAD CI를 별도로 확인하며 운영 통합 및 외부 차단조건이 충족되지 않아 병합·운영 배포를 보류한다.
