"""Durable synthetic receipts for validated patch operator decision intents."""
...
from .patch_operator_decision_intake import (
...
PATCH_DECISION_RECEIPT_SCHEMA_VERSION = 1
PATCH_RECEIPT_STATE = "SYNTHETIC_PATCH_DECISION_RECEIPT_RECORDED"
PATCH_DECISION_RECEIPT_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS synthetic_patch_decision_receipt_metadata (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
    schema_version INTEGER NOT NULL CHECK (schema_version = 1),
    synthetic_only INTEGER NOT NULL CHECK (synthetic_only = 1)
);
CREATE TABLE IF NOT EXISTS synthetic_patch_decision_receipts (
    ...
    decision TEXT NOT NULL CHECK (decision IN (
        'AUTHORIZE_SYNTHETIC_PATCH_DRAFT', 'HOLD', 'REJECT'
    )),
    assessment_json TEXT NOT NULL,
    state TEXT NOT NULL
        CHECK (state = 'SYNTHETIC_PATCH_DECISION_RECEIPT_RECORDED'),
...
CREATE TABLE IF NOT EXISTS synthetic_patch_decision_receipt_audit (
    audit_sequence INTEGER PRIMARY KEY,
    receipt_id TEXT NOT NULL UNIQUE,
    action TEXT NOT NULL
        CHECK (action = 'SYNTHETIC_PATCH_DECISION_INTENT_RECEIPTED'),
    actor_id TEXT NOT NULL
        CHECK (actor_id = 'synthetic:system:patch-decision-receipt-ledger'),
...
class SyntheticPatchDecisionReceipt:
    receipt_id: str
    assessment_id: str
    ...
    def __post_init__(self) -> None:
        if (
            not self.receipt_id.startswith("synthetic:patch-decision-receipt:")
            ...
            or self.state != PATCH_RECEIPT_STATE
            ...
            raise GovernanceRejected("valid non-authorizing patch receipt required")
...
class SyntheticPatchDecisionReceiptLedger:
    """Persists patch validation evidence without recording an actual decision."""
    ...
    def _validate_database_target(database: str) -> None:
        ...
        if not name.startswith("synthetic-") or Path(name).suffix not in {
            ...
            raise GovernanceRejected("synthetic SQLite patch receipt target required")

    def _initialize_schema(self) -> None:
        with self._lock:
            self._connection.executescript(PATCH_DECISION_RECEIPT_SCHEMA_SQL)
            self._connection.execute(
                "INSERT OR IGNORE INTO synthetic_patch_decision_receipt_metadata "
                "(singleton, schema_version, synthetic_only) VALUES (1, ?, 1)",
                (PATCH_DECISION_RECEIPT_SCHEMA_VERSION,),
            )
            if not self.verify_metadata():
                raise GovernanceRejected("unsupported patch receipt schema")

    def verify_metadata(self) -> bool:
        with self._lock:
            rows = self._connection.execute(
                "SELECT singleton, schema_version, synthetic_only "
                "FROM synthetic_patch_decision_receipt_metadata"
            ).fetchall()
            return (
                len(rows) == 1
    ...
    def record_from_intake(
        self,
        intake: SyntheticPatchOperatorDecisionIntake,
        envelope_id: str,
        *,
        recorded_at: datetime,
    ) -> SyntheticPatchDecisionReceipt:
        if recorded_at.tzinfo is None:
            raise GovernanceRejected("timezone-aware patch receipt time required")
        if not isinstance(intake, SyntheticPatchOperatorDecisionIntake):
            raise GovernanceRejected("typed synthetic patch decision intake required")
        if not intake.verify_assessment_chain():
            raise GovernanceRejected("patch decision assessment chain is invalid")
        matches = [
            item for item in intake.assessments if item.envelope_id == envelope_id
        ]
        if len(matches) != 1:
            raise GovernanceRejected("one validated patch decision assessment required")
        assessment = matches[0]
        self._validate_assessment(assessment)
        if recorded_at < assessment.assessed_at:
            raise GovernanceRejected("patch receipt cannot predate assessment")
        receipt_id = "synthetic:patch-decision-receipt:" + assessment.assessment_digest[:32]
        payload = {
        ...
        with self._lock:
            try:
                with self._transaction():
                    if not self.verify_metadata():
                        raise GovernanceRejected("synthetic patch receipt metadata is invalid")
                    existing = self._connection.execute(
                        "SELECT * FROM synthetic_patch_decision_receipts "
                        "WHERE receipt_id = ?", (receipt_id,)
                    ).fetchone()
                    if existing is not None:
                        if (
                            existing["assessment_digest"] != assessment.assessment_digest
                            or existing["envelope_id"] != envelope_id
                        ):
                            raise GovernanceRejected("synthetic patch receipt collision")
                        if not self.verify_record_bindings():
                            raise GovernanceRejected("existing synthetic patch receipt is invalid")
                        if before != intake.evidence()["report_digest"]:
                            raise GovernanceRejected("patch assessment changed during receipting")
                        return self._record_from_row(existing)
                    self._connection.execute(
                        """
                        INSERT INTO synthetic_patch_decision_receipts (
                        ...
                        (
                            ...
                            json.dumps(payload, ensure_ascii=False, sort_keys=True,
                                       separators=(",", ":")),
                            PATCH_RECEIPT_STATE, recorded_at.isoformat(),
                        ),
                    )
                    self._append_audit(receipt_id, assessment.assessment_digest, recorded_at)
                    if before != intake.evidence()["report_digest"]:
                        raise GovernanceRejected("patch assessment changed during receipting")
                    return self._get_record(receipt_id)
            except sqlite3.Error as exc:
                raise GovernanceRejected("synthetic patch receipt transaction failed") from exc

    @staticmethod
    def _validate_assessment(assessment: PatchOperatorDecisionIntentAssessment) -> None:
        if (
            not isinstance(assessment, PatchOperatorDecisionIntentAssessment)
            or assessment.validation_state != "SYNTHETIC_PATCH_DECISION_VALIDATED"
            ...
            raise GovernanceRejected("intact synthetic-only patch assessment required")
    ...
    def _append_audit(
        ...
        latest = self._connection.execute(
            ...
            "FROM synthetic_patch_decision_receipt_audit "
        ...
        value = {
            ...
            "action": "SYNTHETIC_PATCH_DECISION_INTENT_RECEIPTED",
            "actor_id": "synthetic:system:patch-decision-receipt-ledger",
            "evidence_digest": evidence_digest,
            "previous_digest": previous,
            "recorded_at": recorded_at.isoformat(),
        }
        self._connection.execute(
            "INSERT INTO synthetic_patch_decision_receipt_audit "
            "(audit_sequence, receipt_id, action, actor_id, evidence_digest, "
            "previous_digest, audit_digest, recorded_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (sequence, receipt_id, value["action"], value["actor_id"],
             evidence_digest, previous, canonical_digest(value), recorded_at.isoformat()),
        )

    @staticmethod
    def _validate_stored_assessment(payload: object) -> None:
        if not isinstance(payload, dict):
            raise GovernanceRejected("stored patch assessment object required")
        assessment = PatchOperatorDecisionIntentAssessment(
            int(payload["sequence"]), str(payload["assessment_id"]),
            str(payload["envelope_id"]), str(payload["envelope_digest"]),
            str(payload["nonce"]), str(payload["packet_id"]),
            str(payload["packet_digest"]), str(payload["operator_id"]),
            SyntheticPatchOperatorDecision(str(payload["decision"])),
            datetime.fromisoformat(str(payload["assessed_at"])),
            str(payload["previous_digest"]), str(payload["assessment_digest"]),
            str(payload["validation_state"]), bool(payload["packet_state_changed"]),
            bool(payload["operator_decision_recorded"]),
            bool(payload["patch_content_present"]), bool(payload["code_change_allowed"]),
        ...
        if assessment.assessment_digest != payload["assessment_digest"]:
            raise GovernanceRejected("stored patch assessment digest mismatch")

    @classmethod
    def _record_from_row(cls, row: sqlite3.Row) -> SyntheticPatchDecisionReceipt:
        try:
            ...
            raise GovernanceRejected("stored synthetic patch receipt is invalid") from exc
        expected = "synthetic:patch-decision-receipt:" + row["assessment_digest"][:32]
        bindings = (
            ("receipt_id", expected), ("assessment_id", payload["assessment_id"]),
            ("assessment_digest", payload["assessment_digest"]),
            ("envelope_id", payload["envelope_id"]),
            ("envelope_digest", payload["envelope_digest"]),
            ("nonce", payload["nonce"]), ("packet_id", payload["packet_id"]),
            ("packet_digest", payload["packet_digest"]),
            ("operator_id", payload["operator_id"]), ("decision", payload["decision"]),
        )
        if any(row[field] != value for field, value in bindings):
            raise GovernanceRejected("stored synthetic patch receipt binding is invalid")
        return SyntheticPatchDecisionReceipt(
            ...
            SyntheticPatchOperatorDecision(row["decision"]), row["state"],
            ...
            bool(row["code_change_allowed"]), bool(row["automatic_application_allowed"]),
            bool(row["safety_baseline_relaxation_allowed"]),
            bool(row["execution_allowed"]), bool(row["production_activation_allowed"]),
        )

    def _get_record(self, receipt_id: str) -> SyntheticPatchDecisionReceipt:
        row = self._connection.execute(
            "SELECT * FROM synthetic_patch_decision_receipts WHERE receipt_id = ?",
            (receipt_id,),
        ).fetchone()
        if row is None:
            raise GovernanceRejected("unknown synthetic patch decision receipt")
        return self._record_from_row(row)

    def get(self, receipt_id: str) -> SyntheticPatchDecisionReceipt:
        with self._lock:
            return self._get_record(receipt_id)
    ...
    def verify_audit_chain(self) -> bool:
        with self._lock:
            rows = self._connection.execute(
                "SELECT * FROM synthetic_patch_decision_receipt_audit "
                "ORDER BY audit_sequence"
            ).fetchall()
            previous = "0" * 64
            for sequence, row in enumerate(rows, start=1):
                value = {
                    "audit_sequence": sequence, "receipt_id": row["receipt_id"],
    ...
    def verify_record_bindings(self) -> bool:
        with self._lock:
            ...
            receipts = self._connection.execute(
                "SELECT * FROM synthetic_patch_decision_receipts ORDER BY receipt_id"
            ).fetchall()
            audits = self._connection.execute(
                "SELECT * FROM synthetic_patch_decision_receipt_audit ORDER BY audit_sequence"
            ).fetchall()
            if len(receipts) != len(audits):
                return False
            by_receipt = {row["receipt_id"]: row for row in audits}
            try:
                for receipt in receipts:
                    self._record_from_row(receipt)
                    audit = by_receipt.get(receipt["receipt_id"])
                    if (
                        audit is None
                        or audit["action"] != "SYNTHETIC_PATCH_DECISION_INTENT_RECEIPTED"
                        or audit["actor_id"]
                        != "synthetic:system:patch-decision-receipt-ledger"
    ...
    def evidence(self) -> dict[str, object]:
        with self._lock:
            counts = {item.value: 0 for item in SyntheticPatchOperatorDecision}
            for row in self._connection.execute(
                "SELECT decision, COUNT(*) AS count FROM synthetic_patch_decision_receipts "
                "GROUP BY decision"
            ):
            ...
            latest = self._connection.execute(
                "SELECT audit_digest FROM synthetic_patch_decision_receipt_audit "
                "ORDER BY audit_sequence DESC LIMIT 1"
            ).fetchone()
            value = {
                "schema": "nurion.pg.synthetic-patch-decision-receipt-ledger-evidence.v1",
                "database_engine": "SQLite",
                "schema_version": PATCH_DECISION_RECEIPT_SCHEMA_VERSION,
                "schema_digest": canonical_digest(PATCH_DECISION_RECEIPT_SCHEMA_SQL),
                ...
                "maximum_state": PATCH_RECEIPT_STATE,
