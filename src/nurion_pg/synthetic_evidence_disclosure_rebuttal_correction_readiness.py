"""Synthetic/in-memory evidence disclosure, rebuttal, and correction readiness #5901-#6300."""
...
WORKSTREAM_NAMES = (
    "RECONSIDERATION_RECEIPT_ANCHOR", "LESSON_RULE_REGISTRY_BINDING",
    "TENANT_DISCLOSURE_DOCUMENT", "NURION_DISCLOSURE_DOCUMENT",
    "UPSTREAM_DISCLOSURE_DOCUMENT", "FOUR_DIRECTION_DISCLOSURE_ROUTE",
    "EVIDENCE_GAP_DISCLOSURE", "SEMANTIC_BINDING_DISCLOSURE",
    "ROLE_CONFLICT_DISCLOSURE", "PARTIAL_BATCH_DISCLOSURE", "LINEAGE_BREAK_DISCLOSURE",
    "COUNTERPARTY_REBUTTAL_DOCUMENT", "CORRECTION_DRAFT_PROVENANCE",
    "APPEND_ONLY_CORRECTION_HOLD_CHAIN", "INDEPENDENT_PACKET_COMPILATION",
    "NON_PUBLICATION_EXECUTION_BOUNDARY",
)
WORKSTREAMS = tuple((5901+i*25, 5925+i*25, n) for i,n in enumerate(WORKSTREAM_NAMES))
CONTROL_ASPECTS = (
...
TOPICS=("EVIDENCE_GAP","SEMANTIC_BINDING","ROLE_CONFLICT","PARTIAL_BATCH","LINEAGE_BREAK")
APPLIED_LESSONS=("ARL-3901-001","ARL-4301-001","ARL-4301-002","ARL-4301-003","ARL-4701-001","ARL-5101-001","ARL-5501-001","ARL-5901-001")
...
@dataclass(frozen=True)
class DisclosureAnchor:
    anchor_id:str; reconsideration_receipt_digest:str; challenge_set_digest:str; lesson_registry_digest:str
    ...
    source_sequence:int; is_latest:bool; source_reviewers:tuple[str,str,str]
...
class CorrectionCase:
    ...
    disclosure_digest:str; rebuttal_digest:str; correction_draft_digest:str; route_binding_digest:str
...
class ReadinessPacket:
    ...
    complete:bool; published:bool; decision_recorded:bool; approval_recorded:bool; activation_recorded:bool
    deployment_recorded:bool; status:str; digest:str

class SyntheticEvidenceDisclosureRebuttalCorrectionReadiness:
    ...
    def add_control(self,cid,workstream,aspect,requirement_ref,fixture_digest,expected_result):
        ...
        with self._lock:
            if not self._valid_control(row): raise GovernanceRejected("valid #5901-#6300 control required")
    ...
    def anchor_source(self,anchor_id,reconsideration_receipt_digest,challenge_set_digest,lesson_registry_digest,
                      remediation_manifest_digest,applied_lesson_ids,applied_rule_ids,party_document_digests,
                      route_binding_digests,source_sequence,is_latest,source_reviewers):
        reviewers=tuple(source_reviewers)
        p=dict(anchor_id=anchor_id,reconsideration_receipt_digest=reconsideration_receipt_digest,
               challenge_set_digest=challenge_set_digest,lesson_registry_digest=lesson_registry_digest,
               ...
               reviewer_lineage_digest=canonical_digest(("RECONSIDERATION_REVIEWER_LINEAGE",reconsideration_receipt_digest,reviewers)),
               status="RECONSIDERATION_RECEIPT_ANCHORED_NOT_PUBLISHED")
        row=DisclosureAnchor(**p,digest=canonical_digest(p))
        with self._lock:
            ...
            self._anchor=row; self._event("RECONSIDERATION_SOURCE_ANCHORED",row.digest); return row

    def add_case(self,case_id,flow,topic,disclosure_digest,rebuttal_digest,correction_draft_digest,discloser,rebutter):
        with self._lock:
            ...
            p=dict(case_id=case_id,flow=flow,topic=topic,source_party=source,counterparty=counterparty,
                   anchor_digest=self._anchor.digest,disclosure_digest=disclosure_digest,rebuttal_digest=rebuttal_digest,
                   correction_draft_digest=correction_draft_digest,route_binding_digest=route,discloser=discloser,
                   rebutter=rebutter,status="CORRECTION_DRAFT_HELD_NOT_PUBLISHED")
            row=CorrectionCase(**p,digest=canonical_digest(p)); key=(flow,topic)
            ...
            self._cases[key]=row; self._event("CORRECTION_CASE_RECORDED",row.digest); self._hold("CORRECTION_REQUIRES_OPERATOR_REVIEW",row.digest)
            return row
    ...
    def finalize(self,packet_id,compiler):
        with self._lock:
            ...
            if not _syn(packet_id,"correction-readiness-packet") or not _syn(compiler,"packet-compiler"):
                raise GovernanceRejected("synthetic compiler required")
            if _identity(compiler) in {_identity(x) for x in self._anchor.source_reviewers}: raise GovernanceRejected("independent compiler required")
            ...
            p=dict(packet_id=packet_id,anchor_digest=self._anchor.digest,case_set_digest=case_set,compiler=compiler,
                   source_reviewers=self._anchor.source_reviewers,complete=True,published=False,decision_recorded=False,
                   approval_recorded=False,activation_recorded=False,deployment_recorded=False,
                   status="CORRECTION_READINESS_PACKET_READY_NOT_PUBLISHED_NOT_DECIDED")
            self._packet=ReadinessPacket(**p,digest=canonical_digest(p)); self._event("READINESS_PACKET_RECORDED",self._packet.digest)
            return self._packet

    def _expected(self,cid): i=cid-5901; return WORKSTREAM_NAMES[i//25],CONTROL_ASPECTS[i%25]
    def _valid_control(self,r):
        p={k:v for k,v in r.__dict__.items() if k!="digest"}
        return (isinstance(r.control_id,int) and not isinstance(r.control_id,bool) and 5901<=r.control_id<=6300
                and self._expected(r.control_id)==(r.workstream,r.aspect)
                and r.requirement_ref==f"synthetic:correction-requirement:{(r.control_id-5901)//25:02}"
    ...
    def _registry_valid(self): return set(self._controls)==set(range(5901,6301)) and all(k==v.control_id and self._valid_control(v) for k,v in self._controls.items())
    def _anchor_valid(self,a):
        p={k:v for k,v in a.__dict__.items() if k!="digest"}; docs=dict(a.party_document_digests)
        expected_routes=tuple((f,canonical_digest(("DISCLOSURE_ROUTE",f,FLOW_PARTIES[f],tuple(a.party_document_digests),a.reconsideration_receipt_digest))) for f in FLOWS)
        return (self._registry_valid() and _syn(a.anchor_id,"disclosure-anchor")
                and all(_hex(x) for x in (a.reconsideration_receipt_digest,a.challenge_set_digest,a.lesson_registry_digest,a.remediation_manifest_digest))
                ...
                and len({_identity(x) for x in a.source_reviewers})==3
                and a.reviewer_lineage_digest==canonical_digest(("RECONSIDERATION_REVIEWER_LINEAGE",a.reconsideration_receipt_digest,a.source_reviewers))
                and a.status=="RECONSIDERATION_RECEIPT_ANCHORED_NOT_PUBLISHED"
    ...
    def _case_valid(self,r,key):
        ...
        route=dict(self._anchor.route_binding_digests).get(r.flow); source_doc=dict(self._anchor.party_document_digests).get(source)
        disclosure=canonical_digest(("DISCLOSURE_DOCUMENT",source,source_doc,r.flow,r.topic,route,self._anchor.challenge_set_digest))
        rebuttal=canonical_digest(("REBUTTAL_DOCUMENT",counterparty,r.flow,r.topic,disclosure,route))
        correction=canonical_digest(("CORRECTION_DRAFT",source,counterparty,r.flow,r.topic,disclosure,rebuttal,route))
        return (key==(r.flow,r.topic) and r.flow in FLOWS and r.topic in TOPICS and _syn(r.case_id,"correction-case")
                and (r.source_party,r.counterparty)==(source,counterparty) and r.anchor_digest==self._anchor.digest
                and (r.disclosure_digest,r.rebuttal_digest,r.correction_draft_digest)==(disclosure,rebuttal,correction)
                and r.route_binding_digest==route and r.discloser==f"synthetic:discloser:{source}"
                and r.rebutter==f"synthetic:rebutter:{counterparty}" and r.status=="CORRECTION_DRAFT_HELD_NOT_PUBLISHED"
                and r.digest==canonical_digest(p))
    ...
    def _integrity(self):
        ...
        if self._packet:
            ...
            if (not _syn(r.packet_id,"correction-readiness-packet") or r.anchor_digest!=self._anchor.digest or r.case_set_digest!=case_set
                    or not _syn(r.compiler,"packet-compiler") or r.source_reviewers!=self._anchor.source_reviewers
                    or _identity(r.compiler) in {_identity(x) for x in r.source_reviewers} or not r.complete or r.published
                    or r.decision_recorded or r.approval_recorded or r.activation_recorded or r.deployment_recorded
                    or r.status!="CORRECTION_READINESS_PACKET_READY_NOT_PUBLISHED_NOT_DECIDED" or r.digest!=canonical_digest(p)):return False
        expected=[]
        if self._anchor:expected.append(("RECONSIDERATION_SOURCE_ANCHORED",self._anchor.digest))
        expected.extend(("CORRECTION_CASE_RECORDED",x.digest) for x in self._cases.values())
        ...
        holds=[("CORRECTION_REQUIRES_OPERATOR_REVIEW",x.digest) for x in self._cases.values()]
    ...
    def evidence(self):
        ...
        matrix=WORKSTREAMS==tuple((5901+i*25,5925+i*25,n) for i,n in enumerate(WORKSTREAM_NAMES)) and len(CONTROL_ASPECTS)==25
        return {"range":[5901,6300],"control_count":400,"workstream_count":16,"controls_per_workstream":25,
                "control_matrix_valid":matrix,"registered_control_count":len(self._controls),"case_count":len(self._cases),
                "hold_count":len(self._holds),"event_count":len(self._events),"integrity_valid":ok,"complete_correction_readiness_evidence":complete and ok,
                "maximum_state":"CORRECTION_READINESS_PACKET_READY_NOT_PUBLISHED_NOT_DECIDED" if self._packet else "CORRECTION_READINESS_INCOMPLETE",
                ...
                "approval_recorded":False,"activation_recorded":False}
