"""Durable synthetic receipts for validated activation intent."""
...
_RECEIPT_PREFIX = "synthetic:limited-promotion-activation-receipt:"
_ACTOR_ID = "synthetic:system:limited-promotion-activation-receipt-ledger"


ACTIVATION_RECEIPT_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS synthetic_activation_receipt_metadata (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
    schema_version INTEGER NOT NULL CHECK (schema_version = 1),
    synthetic_only INTEGER NOT NULL CHECK (synthetic_only = 1)
);
CREATE TABLE IF NOT EXISTS synthetic_activation_receipts (
    ...
    decision TEXT NOT NULL CHECK (decision IN (
        'AUTHORIZE_SYNTHETIC_LIMITED_PROMOTION_ACTIVATION', 'HOLD', 'REJECT'
    )),
    ...
    activation_recorded INTEGER NOT NULL DEFAULT 0
        CHECK (activation_recorded = 0),
    packet_state_changed INTEGER NOT NULL DEFAULT 0
        CHECK (packet_state_changed = 0),
...
CREATE TABLE IF NOT EXISTS synthetic_activation_receipt_audit (
    audit_sequence INTEGER PRIMARY KEY,
    receipt_id TEXT NOT NULL UNIQUE,
    action TEXT NOT NULL CHECK (
        action = 'SYNTHETIC_LIMITED_PROMOTION_ACTIVATION_INTENT_RECEIPTED'
    ),
    actor_id TEXT NOT NULL CHECK (
        actor_id = 'synthetic:system:limited-promotion-activation-receipt-ledger'
    ),
...
class SyntheticPatchDraftLimitedPromotionActivationReceipt:
    receipt_id: str
    assessment_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.state != ACTIVATION_RECEIPT_STATE
            or self.recorded_at.tzinfo is None
            or any(forbidden)
        ):
            raise GovernanceRejected(
                "valid non-authorizing activation receipt required"
            )
...
class SyntheticPatchDraftLimitedPromotionActivationReceiptLedger:
    """Persists verified evidence without recording a real decision."""
    ...
    def _validate_database_target(database: str) -> None:
        ...
        if not path.name.startswith("synthetic-") or path.suffix not in {
            ...
            raise GovernanceRejected("synthetic SQLite activation receipt target required")

    def _initialize_schema(self) -> None:
        with self._lock:
            self._connection.executescript(ACTIVATION_RECEIPT_SCHEMA_SQL)
            self._connection.execute(
                "INSERT OR IGNORE INTO synthetic_activation_receipt_metadata "
                "(singleton, schema_version, synthetic_only) VALUES (1, ?, 1)",
                (ACTIVATION_RECEIPT_SCHEMA_VERSION,),
            )
            if not self.verify_metadata():
                raise GovernanceRejected("unsupported activation receipt schema")

    def verify_metadata(self) -> bool:
        with self._lock:
            rows = self._connection.execute(
                "SELECT * FROM synthetic_activation_receipt_metadata"
            ).fetchall()
            return (
                len(rows) == 1
                and rows[0]["singleton"] == 1
                and rows[0]["schema_version"] == ACTIVATION_RECEIPT_SCHEMA_VERSION
    ...
    def _validate_assessment(
        ...
        if (
            ...
            raise GovernanceRejected("intact synthetic activation assessment required")

    def record_from_intake(
        self,
        intake: SyntheticPatchDraftLimitedPromotionActivationIntake,
        envelope_id: str,
        *,
        recorded_at: datetime,
    ) -> SyntheticPatchDraftLimitedPromotionActivationReceipt:
        if recorded_at.tzinfo is None:
            raise GovernanceRejected("timezone-aware activation receipt required")
        if not isinstance(
            intake,
            SyntheticPatchDraftLimitedPromotionActivationIntake,
        ):
            raise GovernanceRejected("typed activation intake required")
        if not intake.verify_assessment_chain():
            raise GovernanceRejected("activation assessment chain is invalid")
        matches = [
            item for item in intake.assessments if item.envelope_id == envelope_id
        ]
        if len(matches) != 1:
            raise GovernanceRejected("one validated activation assessment required")
        assessment = matches[0]
        self._validate_assessment(assessment)
        if recorded_at < assessment.assessed_at:
            raise GovernanceRejected("activation receipt cannot predate assessment")
        ...
        with self._lock:
            try:
                with self._transaction():
                    if not self.verify_metadata():
                        raise GovernanceRejected("activation receipt metadata is invalid")
                    existing = self._connection.execute(
                        "SELECT * FROM synthetic_activation_receipts "
                        "WHERE receipt_id = ?",
                        (receipt_id,),
                    ).fetchone()
                    if existing is not None:
                        if (
                            ...
                            raise GovernanceRejected("activation receipt collision")
                        if not self.verify_record_bindings():
                            raise GovernanceRejected(
                                "existing activation receipt is invalid"
                            )
                        if before != intake.evidence()["report_digest"]:
                            raise GovernanceRejected(
                                "activation assessment changed during receipting"
                            )
                        return self._record_from_row(existing)
                    self._connection.execute(
                        """
                        INSERT INTO synthetic_activation_receipts (
                            receipt_id, assessment_id, assessment_digest,
                            envelope_id, envelope_digest, nonce, packet_id,
                            packet_digest, manifest_id, operator_id, decision,
                            assessment_json, state, recorded_at
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """,
                        (
                            receipt_id,
                ...
                raise GovernanceRejected(
                    "activation receipt transaction failed"
                ) from exc
    ...
    def _append_audit(
        ...
        latest = self._connection.execute(
            "SELECT * FROM synthetic_activation_receipt_audit "
        ...
        self._connection.execute(
            "INSERT INTO synthetic_activation_receipt_audit "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                sequence,
                receipt_id,
    ...
    def _assessment_from_payload(
        ...
        if not isinstance(payload, dict):
            raise GovernanceRejected("stored activation assessment required")
        try:
            assessment = PatchDraftLimitedPromotionActivationIntentAssessment(
                ...
                assessment_digest=str(payload["assessment_digest"]),
                validation_state=str(payload["validation_state"]),
                packet_state_changed=bool(payload["packet_state_changed"]),
                operator_decision_recorded=bool(
                    payload["operator_decision_recorded"]
                ),
                activation_recorded=bool(payload["activation_recorded"]),
                candidate_content_present=bool(
                    payload["candidate_content_present"]
                ),
                patch_content_present=bool(payload["patch_content_present"]),
                diff_content_present=bool(payload["diff_content_present"]),
                source_code_changed=bool(payload["source_code_changed"]),
                filesystem_written=bool(payload["filesystem_written"]),
                automatic_application_allowed=bool(
                    payload["automatic_application_allowed"]
                ),
                rollback_execution_allowed=bool(
                    payload["rollback_execution_allowed"]
                ),
                safety_baseline_relaxation_allowed=bool(
                    payload["safety_baseline_relaxation_allowed"]
                ),
                execution_allowed=bool(payload["execution_allowed"]),
                production_activation_allowed=bool(
                    payload["production_activation_allowed"]
                ),
            )
        except (KeyError, TypeError, ValueError) as exc:
            raise GovernanceRejected("stored activation assessment is invalid") from exc
        return assessment
    ...
    def _record_from_row(
        ...
        except (GovernanceRejected, json.JSONDecodeError) as exc:
            raise GovernanceRejected("stored activation receipt is invalid") from exc
        ...
        bindings = (
            ...
            ("envelope_digest", assessment.envelope_digest),
        ...
        if any(row[field] != value for field, value in bindings):
            raise GovernanceRejected("stored activation receipt binding is invalid")
        return SyntheticPatchDraftLimitedPromotionActivationReceipt(
            row["receipt_id"],
            row["assessment_id"],
            ...
            row["manifest_id"],
            row["operator_id"],
            SyntheticPatchDraftLimitedPromotionActivationDecision(row["decision"]),
            row["state"],
            datetime.fromisoformat(row["recorded_at"]),
            bool(row["actual_operator_decision_recorded"]),
            bool(row["activation_recorded"]),
    ...
    def _get_record(
        ...
        row = self._connection.execute(
            "SELECT * FROM synthetic_activation_receipts WHERE receipt_id = ?",
            (receipt_id,),
        ).fetchone()
        if row is None:
            raise GovernanceRejected("unknown activation receipt")
    ...
    def evidence(self) -> dict[str, object]:
        with self._lock:
            counts = {
                item.value: 0
                for item in SyntheticPatchDraftLimitedPromotionActivationDecision
            }
            for row in self._connection.execute(
                "SELECT decision, COUNT(*) AS count FROM "
                "synthetic_activation_receipts GROUP BY decision"
            ):
                counts[row["decision"]] = row["count"]
            receipts = [
                self._record_from_row(row)
                for row in self._connection.execute(
                    "SELECT * FROM synthetic_activation_receipts ORDER BY receipt_id"
                )
            ]
            latest = self._connection.execute(
                "SELECT audit_digest FROM synthetic_activation_receipt_audit "
                "ORDER BY audit_sequence DESC LIMIT 1"
            ).fetchone()
            value = {
                "schema": (
                    "nurion.pg.synthetic-limited-promotion-activation-"
                    "receipt-ledger-evidence.v1"
                ),
                "mode": "UNREGISTERED_SYNTHETIC_ONLY",
                "database_engine": "SQLite",
                "schema_version": ACTIVATION_RECEIPT_SCHEMA_VERSION,
                "schema_digest": canonical_digest(ACTIVATION_RECEIPT_SCHEMA_SQL),
                ...
                "maximum_state": ACTIVATION_RECEIPT_STATE,
