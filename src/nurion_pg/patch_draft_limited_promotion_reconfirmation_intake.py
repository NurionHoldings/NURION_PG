"""Synthetic-only validation of limited promotion reconfirmation intent."""
...
LIMITED_PROMOTION_RECONFIRMATION_VALIDATION_STATE = (
    "SYNTHETIC_PATCH_DRAFT_LIMITED_PROMOTION_RECONFIRMATION_VALIDATED"
)


class SyntheticPatchDraftLimitedPromotionReconfirmationDecision(StrEnum):
...
def _assessment_id(envelope_digest: str) -> str:
    return (
        "synthetic:patch-draft-limited-promotion-reconfirmation-assessment:"
        + envelope_digest[:32]
    )


@dataclass(frozen=True)
class SyntheticPatchDraftLimitedPromotionReconfirmationEnvelope:
    envelope_id: str
    packet_id: str
    packet_digest: str
    operator_id: str
    ...
    def __post_init__(self) -> None:
        envelope_prefix = (
            "synthetic:patch-draft-limited-promotion-reconfirmation-envelope:"
        )
        packet_prefix = (
            "synthetic:patch-draft-limited-promotion-reconfirmation:"
        )
        nonce_prefix = (
            "synthetic:patch-draft-limited-promotion-reconfirmation-nonce:"
        )
        if (
            ...
            or self.decision.value
            not in LIMITED_PROMOTION_RECONFIRMATION_ALLOWED_DECISIONS
            ...
            raise GovernanceRejected(
                "complete synthetic limited promotion reconfirmation envelope required"
            )
...
@dataclass(frozen=True)
class PatchDraftLimitedPromotionReconfirmationIntentAssessment:
    sequence: int
    assessment_id: str
    envelope_id: str
    envelope_digest: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.validation_state
            != LIMITED_PROMOTION_RECONFIRMATION_VALIDATION_STATE
            or any(forbidden)
            or self.assessment_digest != canonical_digest(self.digest_value())
        ):
            raise GovernanceRejected(
                "valid non-recording limited promotion reconfirmation assessment required"
            )
...
class SyntheticPatchDraftLimitedPromotionReconfirmationIntake:
    ...
    def __init__(self, registry: SyntheticOperatorKeyRegistry) -> None:
        if not isinstance(registry, SyntheticOperatorKeyRegistry):
            raise GovernanceRejected(
                "typed synthetic operator key registry required"
            )
        ...
        self._by_envelope: dict[
            str, PatchDraftLimitedPromotionReconfirmationIntentAssessment
        ] = {}
    ...
    def assessments(
        self,
    ) -> tuple[PatchDraftLimitedPromotionReconfirmationIntentAssessment, ...]:
        with self._lock:
    ...
    def assess(
        ...
        shadow_id: str,
        envelope: SyntheticPatchDraftLimitedPromotionReconfirmationEnvelope,
        *,
        ...
        if not isinstance(
            ...
            raise GovernanceRejected(
                "typed limited promotion reconfirmation packet book required"
            )
        if not isinstance(
            envelope,
            ...
            raise GovernanceRejected(
                "typed synthetic limited promotion reconfirmation envelope required"
            )
        if received_at < envelope.issued_at - MAX_FUTURE_SKEW:
            raise GovernanceRejected("reconfirmation envelope is from the future")
        if received_at > envelope.expires_at:
            raise GovernanceRejected("reconfirmation envelope has expired")
        ...
        if (
            ...
            raise GovernanceRejected(
                "reconfirmation envelope is not bound to current packet"
            )
        key = self._registry.resolve(
            envelope.key_id, issued_at=envelope.issued_at
        )
        if key.operator_id != envelope.operator_id:
            raise GovernanceRejected("reconfirmation key identity mismatch")
        ...
        if not hmac.compare_digest(expected, envelope.signature):
            raise GovernanceRejected("reconfirmation signature mismatch")
        ...
        with self._lock:
            ...
            if not self.verify_assessment_chain():
                raise GovernanceRejected(
                    "existing reconfirmation intent evidence is invalid"
                )
            existing = self._by_envelope.get(envelope.envelope_id)
            if existing is not None:
                if existing.envelope_digest != envelope_digest:
                    raise GovernanceRejected(
                        "reconfirmation envelope idempotency mismatch"
                    )
                assessment = existing
            else:
                if envelope.nonce in self._nonces:
                    raise GovernanceRejected("reconfirmation nonce replay")
            ...
            if before != packet_book.evidence()["report_digest"]:
                ...
                raise GovernanceRejected(
                    "reconfirmation packet changed during validation"
                )
        return assessment
        ...
        with self._lock:
            ...
            value = {
                "schema": (
                    ...
                    "reconfirmation-intake-evidence.v1"
                ),
                ...
                "operator_reconfirmation_recording_method_present": False,
                "packet_state_changed": False,
                "candidate_content_present": False,
                "patch_content_present": False,
                "diff_content_present": False,
