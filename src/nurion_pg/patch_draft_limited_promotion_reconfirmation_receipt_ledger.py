"""Durable synthetic receipts for validated reconfirmation intent."""
...
from .patch_draft_limited_promotion_reconfirmation_intake import (
    ...
    PatchDraftLimitedPromotionReconfirmationIntentAssessment,
    SyntheticPatchDraftLimitedPromotionReconfirmationDecision,
    SyntheticPatchDraftLimitedPromotionReconfirmationIntake,
)


LIMITED_PROMOTION_RECONFIRMATION_RECEIPT_SCHEMA_VERSION = 1
LIMITED_PROMOTION_RECONFIRMATION_RECEIPT_STATE = (
    "SYNTHETIC_PATCH_DRAFT_LIMITED_PROMOTION_RECONFIRMATION_RECEIPT_RECORDED"
)
_RECEIPT_PREFIX = (
    "synthetic:patch-draft-limited-promotion-reconfirmation-receipt:"
)
_ACTOR_ID = (
    "synthetic:system:patch-draft-limited-promotion-"
    "reconfirmation-receipt-ledger"
)


LIMITED_PROMOTION_RECONFIRMATION_RECEIPT_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS synthetic_limited_promotion_reconfirmation_metadata (
    singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
    schema_version INTEGER NOT NULL CHECK (schema_version = 1),
    synthetic_only INTEGER NOT NULL CHECK (synthetic_only = 1)
);
CREATE TABLE IF NOT EXISTS synthetic_limited_promotion_reconfirmation_receipts (
    ...
    decision TEXT NOT NULL CHECK (decision IN (
        'RECONFIRM_SYNTHETIC_PATCH_DRAFT_LIMITED_PROMOTION', 'HOLD', 'REJECT'
    )),
    ...
    actual_operator_reconfirmation_recorded INTEGER NOT NULL DEFAULT 0
        CHECK (actual_operator_reconfirmation_recorded = 0),
...
CREATE TABLE IF NOT EXISTS synthetic_limited_promotion_reconfirmation_audit (
    audit_sequence INTEGER PRIMARY KEY,
    receipt_id TEXT NOT NULL UNIQUE,
    action TEXT NOT NULL CHECK (
        action = 'SYNTHETIC_LIMITED_PROMOTION_RECONFIRMATION_INTENT_RECEIPTED'
    ),
    actor_id TEXT NOT NULL CHECK (
        actor_id =
        'synthetic:system:patch-draft-limited-promotion-reconfirmation-receipt-ledger'
    ),
...
class SyntheticPatchDraftLimitedPromotionReconfirmationReceipt:
    receipt_id: str
    assessment_id: str
    ...
    shadow_id: str
    operator_id: str
    decision: SyntheticPatchDraftLimitedPromotionReconfirmationDecision
    state: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.state != LIMITED_PROMOTION_RECONFIRMATION_RECEIPT_STATE
            or self.recorded_at.tzinfo is None
            or any(forbidden)
        ):
            raise GovernanceRejected(
                "valid non-authorizing limited promotion reconfirmation receipt required"
            )
...
class SyntheticPatchDraftLimitedPromotionReconfirmationReceiptLedger:
    """Persists verified evidence without recording an actual reconfirmation."""
    ...
    def _validate_database_target(database: str) -> None:
        ...
        if not path.name.startswith("synthetic-") or path.suffix not in {
            ...
            raise GovernanceRejected(
                "synthetic SQLite reconfirmation receipt target required"
            )
    ...
    def _initialize_schema(self) -> None:
        with self._lock:
            ...
            self._connection.execute(
                ...
                "synthetic_limited_promotion_reconfirmation_metadata "
                "(singleton, schema_version, synthetic_only) VALUES (1, ?, 1)",
                (LIMITED_PROMOTION_RECONFIRMATION_RECEIPT_SCHEMA_VERSION,),
            )
            if not self.verify_metadata():
                raise GovernanceRejected(
                    "unsupported limited promotion reconfirmation receipt schema"
                )
    ...
    def record_from_intake(
        ...
        *,
        recorded_at: datetime,
    ) -> SyntheticPatchDraftLimitedPromotionReconfirmationReceipt:
        if recorded_at.tzinfo is None:
            raise GovernanceRejected(
                "timezone-aware limited promotion reconfirmation receipt required"
            )
        if not isinstance(
            intake,
            SyntheticPatchDraftLimitedPromotionReconfirmationIntake,
        ):
            raise GovernanceRejected("typed reconfirmation intake required")
        if not intake.verify_assessment_chain():
            raise GovernanceRejected("reconfirmation assessment chain is invalid")
        matches = [
            item for item in intake.assessments if item.envelope_id == envelope_id
        ]
        if len(matches) != 1:
            raise GovernanceRejected(
                "one validated reconfirmation assessment required"
            )
        assessment = matches[0]
        self._validate_assessment(assessment)
        if recorded_at < assessment.assessed_at:
            raise GovernanceRejected(
                "reconfirmation receipt cannot predate assessment"
            )
        ...
        with self._lock:
            try:
                with self._transaction():
                    if not self.verify_metadata():
                        raise GovernanceRejected(
                            "reconfirmation receipt metadata is invalid"
                        )
                    existing = self._connection.execute(
                        "SELECT * FROM "
                        "synthetic_limited_promotion_reconfirmation_receipts "
                        "WHERE receipt_id = ?",
                        (receipt_id,),
                    ).fetchone()
                    if existing is not None:
                        if (
                            ...
                            raise GovernanceRejected(
                                "reconfirmation receipt collision"
                            )
                        if not self.verify_record_bindings():
                            raise GovernanceRejected(
                                "existing reconfirmation receipt is invalid"
                            )
                        if before != intake.evidence()["report_digest"]:
                            raise GovernanceRejected(
                                "assessment changed during receipting"
                            )
                        return self._record_from_row(existing)
                    self._connection.execute(
                        """
                        INSERT INTO synthetic_limited_promotion_reconfirmation_receipts (
                            receipt_id, assessment_id, assessment_digest,
                            envelope_id, envelope_digest, nonce, packet_id,
                            packet_digest, shadow_id, operator_id, decision,
                ...
                raise GovernanceRejected(
                    "reconfirmation receipt transaction failed"
                ) from exc

    @staticmethod
    def _validate_assessment(
        assessment: PatchDraftLimitedPromotionReconfirmationIntentAssessment,
    ) -> None:
        if (
            not isinstance(
                assessment,
            ...
            or any(
                (
                    ...
                    assessment.operator_reconfirmation_recorded,
            ...
            raise GovernanceRejected(
                "intact synthetic reconfirmation assessment required"
            )
    ...
    def _append_audit(
        ...
        latest = self._connection.execute(
            "SELECT * FROM "
            "synthetic_limited_promotion_reconfirmation_audit "
        ...
        value = {
            ...
            "action": (
                "SYNTHETIC_LIMITED_PROMOTION_RECONFIRMATION_INTENT_RECEIPTED"
            ),
            "actor_id": _ACTOR_ID,
        ...
        self._connection.execute(
            "INSERT INTO synthetic_limited_promotion_reconfirmation_audit "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                sequence,
                receipt_id,
    ...
    def _assessment_from_payload(
        ...
        if not isinstance(payload, dict):
            raise GovernanceRejected("stored reconfirmation assessment required")
        assessment = PatchDraftLimitedPromotionReconfirmationIntentAssessment(
            ...
            bool(payload["operator_reconfirmation_recorded"]),
        ...
        if assessment.assessment_digest != payload["assessment_digest"]:
            raise GovernanceRejected(
                "stored reconfirmation assessment digest mismatch"
            )
        return assessment
    ...
    def _record_from_row(
        ...
        try:
            ...
            raise GovernanceRejected(
                "stored reconfirmation receipt is invalid"
            ) from exc
        ...
        if any(row[field] != value for field, value in bindings):
            raise GovernanceRejected(
                "stored reconfirmation receipt binding is invalid"
            )
        return SyntheticPatchDraftLimitedPromotionReconfirmationReceipt(
            row["receipt_id"],
            row["assessment_id"],
            ...
            bool(row["actual_operator_reconfirmation_recorded"]),
    ...
    def _get_record(
        ...
        if row is None:
            raise GovernanceRejected("unknown reconfirmation receipt")
        ...
        with self._lock:
            ...
            value = {
                "schema": (
                    ...
                    "reconfirmation-receipt-ledger-evidence.v1"
                ),
                ...
                "actual_operator_reconfirmation_recording_method_present": False,
