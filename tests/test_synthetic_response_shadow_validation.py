...
class Tests(unittest.TestCase):
 ...
 def test_sequence_and_stale_version_fail_closed(self):
  ...
  s.admit("SYNTHETIC_RISK_RESPONSE_PLAN_COMPLETED",(),"synthetic:key:a")
 ...
 def test_idempotency_replay_conflict(self):
  s=ResponseShadowValidation(h(b"p"),h(b"q"));a=s.admit("SYNTHETIC_RISK_RESPONSE_PLAN_COMPLETED",(),"synthetic:key:a");self.assertIs(a,s.admit("SYNTHETIC_RISK_RESPONSE_PLAN_COMPLETED",(),"synthetic:key:a"))
  with self.assertRaises(GovernanceRejected):s.admit("BAD",(),"synthetic:key:a")
 def test_duplicate_and_metric_bounds(self):
  s=ResponseShadowValidation(h(b"p"),h(b"q"));s.admit("SYNTHETIC_RISK_RESPONSE_PLAN_COMPLETED",(),"synthetic:key:a")
