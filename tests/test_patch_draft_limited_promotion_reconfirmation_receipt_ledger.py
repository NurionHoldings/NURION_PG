...
class PatchDraftLimitedPromotionReconfirmationReceiptLedgerTests(
    unittest.TestCase
):
    def test_requires_synthetic_sqlite_target_and_typed_intake(self):
        for target in (
            "reconfirmation-receipts.sqlite3",
            "file:synthetic-reconfirmation.sqlite3",
            "postgresql://prod/reconfirmation",
        ):
    ...
    def test_validated_intent_is_receipted_without_authority(self):
        ...
        self.assertEqual(receipt.shadow_id, assessment.shadow_id)
        self.assertEqual(
            receipt.state,
            LIMITED_PROMOTION_RECONFIRMATION_RECEIPT_STATE,
        )
        for value in (
            receipt.actual_operator_reconfirmation_recorded,
            receipt.packet_state_changed,
            receipt.candidate_content_present,
            ...
            receipt.synthetic_activation_allowed,
    ...
    def test_unknown_tampered_and_invalid_times_are_blocked(self):
        ...
        with self.assertRaises(GovernanceRejected):
            ledger.record_from_intake(
                intake,
                "synthetic:patch-draft-limited-promotion-"
                "reconfirmation-envelope:unknown",
                recorded_at=NOW,
            )
    ...
    def test_one_packet_and_shadow_cannot_create_multiple_receipts(self):
        resources, docket, record, packet_book, packet = packet_source()
        intake = SyntheticPatchDraftLimitedPromotionReconfirmationIntake(
            key_registry()
        )
        first = reconfirmation_envelope(packet)
        second = reconfirmation_envelope(
            packet,
            envelope_id=(
                "synthetic:patch-draft-limited-promotion-"
        ...
        with self.assertRaises(GovernanceRejected):
            ledger.record_from_intake(
                intake, second.envelope_id, recorded_at=NOW
            )
        self.assertEqual(
            ledger._connection.execute(
                "SELECT COUNT(*) FROM "
    ...
    def test_restart_preserves_receipt_and_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            path = (
                Path(directory)
                / "synthetic-reconfirmation-receipts.sqlite3"
            )
            ledger = (
                SyntheticPatchDraftLimitedPromotionReconfirmationReceiptLedger(
                    path
    ...
    def test_stored_assessment_and_audit_tampering_are_detected(self):
        ...
        ledger._connection.execute(
            "UPDATE synthetic_limited_promotion_reconfirmation_receipts "
            "SET assessment_json = '{}'"
        )
        with self.assertRaises(GovernanceRejected):
    ...
    def test_evidence_proves_synthetic_receipt_only(self):
        ...
        self.assertEqual(
            evidence["receipt_digests"], [receipt.receipt_digest()]
        )
        self.assertEqual(
    ...
    def test_deleted_metadata_fails_closed(self):
        ...
        ledger._connection.execute(
            "DELETE FROM synthetic_limited_promotion_reconfirmation_metadata"
        )
