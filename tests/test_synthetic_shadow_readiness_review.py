...
class Tests(unittest.TestCase):
 ...
 def test_sequence_and_stale_version_fail(self):
  ...
  r.admit("SYNTHETIC_RESPONSE_SHADOW_VALIDATION_COMPLETED","HOLD",False,True,("CEILING_BREACH","FAIRNESS_BREACH"),"synthetic:key:a")
  with self.assertRaises(GovernanceRejected):r.blockers(("ROLLBACK_TRIGGERED",),"synthetic:key:b",0)
 def test_idempotency_conflict(self):
  r=ShadowReadinessReview(h(b"v"),h(b"p"));a=r.admit("SYNTHETIC_RESPONSE_SHADOW_VALIDATION_COMPLETED","HOLD",False,True,("CEILING_BREACH","FAIRNESS_BREACH"),"synthetic:key:a");self.assertIs(a,r.admit("SYNTHETIC_RESPONSE_SHADOW_VALIDATION_COMPLETED","HOLD",False,True,("CEILING_BREACH","FAIRNESS_BREACH"),"synthetic:key:a"))
  with self.assertRaises(GovernanceRejected):r.admit("SYNTHETIC_RESPONSE_SHADOW_VALIDATION_COMPLETED","REVISE",False,True,("CEILING_BREACH","FAIRNESS_BREACH"),"synthetic:key:a")
 def test_eligible_consistency_and_blockers(self):
  with self.assertRaises(GovernanceRejected):ShadowReadinessReview(h(b"v"),h(b"p")).admit("SYNTHETIC_RESPONSE_SHADOW_VALIDATION_COMPLETED","ELIGIBLE_DRAFT",False,False,("CEILING_BREACH","FAIRNESS_BREACH"),"synthetic:key:a")
 ...
 def test_blocker_submission_and_trigger_plan_binding(self):
  a=ShadowReadinessReview(h(b"v"),h(b"p"));a.admit("SYNTHETIC_RESPONSE_SHADOW_VALIDATION_COMPLETED","HOLD",False,True,("CEILING_BREACH","FAIRNESS_BREACH"),"synthetic:key:a")
  with self.assertRaises(GovernanceRejected):a.blockers(("NONE",),"synthetic:key:none",1)
  b=complete();b.stages[0]=type(b.stages[0])(b.stages[0].feature,b.stages[0].state,{**b.stages[0].payload,"recommendation":"HOLD"},b.stages[0].previous_digest,b.stages[0].digest,b.stages[0].key);b.stages=b.stages[:11]
  with self.assertRaises(GovernanceRejected):b.packet("synthetic:operator:1","SUBMIT_FOR_OPERATOR_REVIEW","synthetic:key:hold-submit",11)
  c=complete();c.stages=c.stages[:6]
  with self.assertRaises(GovernanceRejected):c.rollback(("DATA_DRIFT",),("DATA_DRIFT",),"synthetic:key:wrong-plan",6)
