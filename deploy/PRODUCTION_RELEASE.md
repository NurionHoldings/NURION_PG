...
3. staging의 격리된 PostgreSQL 백업·복구를 검증하고, 독립된 1회성 작업에서 `NURION_PG_MIGRATION_JOB=approved-one-off`와 `NURION_PG_DATABASE_URL`을 제공해 `python scripts/migrate_ops_database.py`를 실행합니다. 운영 API 프로세스에는 migration 권한을 부여하지 않습니다.
