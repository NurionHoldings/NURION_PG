...
def main():
 rb=REGISTRY.read_bytes();mb=MANIFEST.read_bytes();gb=GUIDANCE.read_bytes();live=validate(json.loads(rb),declared_remediations(json.loads(mb)),discovered_negative_tests());lessons,stage_digest=validated_stage_snapshot(live,APPLIED_LESSONS,"ARKAON-LESSONS-12301",(9901,12700));rules=tuple(x["id"] for x in json.loads(gb)["rules"])
