"""Generate deterministic evidence for patch operator decision packets."""
...
def main() -> None:
    policy = read_json(
        "config/patch-operator-decision-packet-policy.json"
    )
    forbidden = (
    ...
    if (
        ...
        or policy["requested_scope"] != PATCH_PACKET_SCOPE
        ...
        raise SystemExit("patch operator decision packet policy drift detected")

    findings = sha256(b"patch operator decision packet evidence").hexdigest()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        report = patch_packet_book.evidence()
    ...
    if (
        ...
        or report["requested_scope"] != PATCH_PACKET_SCOPE
        ...
        raise SystemExit("patch operator decision packet evidence boundary failed")

    output = ROOT / "build/synthetic-patch-operator-packet-evidence.json"
    output.parent.mkdir(exist_ok=True)
    payload = json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2) + "\n"
    output.write_text(payload, encoding="utf-8")
    digest = sha256(payload.encode("utf-8")).hexdigest()
    output.with_suffix(".json.sha256").write_text(digest + "\n", encoding="ascii")
    print(f"synthetic patch operator packet: PASS {digest}")
