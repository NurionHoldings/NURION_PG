"""Generate evidence for limited promotion reconfirmation intent validation."""
...
def main() -> tuple[
    ...
    policy = read_json(
        ...
        "reconfirmation-intake-policy.json"
    )
    forbidden = (
        "signing_allowed",
    ...
    if (
        ...
        or policy["required_packet_scope"]
        != LIMITED_PROMOTION_RECONFIRMATION_PACKET_SCOPE
        ...
        raise SystemExit(
            "limited promotion reconfirmation intake policy drift detected"
        )
    ...
    signing_value = {
        "envelope_id": (
            ...
            "reconfirmation-envelope:evidence"
        ),
    ...
    if (
        ...
        or assessment.shadow_id != packet.shadow_id
        ...
        raise SystemExit(
            "limited promotion reconfirmation intake evidence boundary failed"
        )

    output = (
        ROOT
        / "build/patch-draft-limited-promotion-"
        "reconfirmation-intake-evidence.json"
    )
    ...
    print(
        "patch draft limited promotion reconfirmation intake: "
        f"PASS {digest}"
    )
