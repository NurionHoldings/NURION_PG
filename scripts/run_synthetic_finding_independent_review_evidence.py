...
def main():
    registry_bytes=REGISTRY.read_bytes();manifest_bytes=MANIFEST.read_bytes();guidance_bytes=GUIDANCE.read_bytes();live=validate(json.loads(registry_bytes),declared_remediations(json.loads(manifest_bytes)),discovered_negative_tests());lessons,stage_digest=validated_stage_snapshot(live,APPLIED_LESSONS,"ARKAON-LESSONS-12301",(9901,12700));rules=tuple(x["id"] for x in json.loads(guidance_bytes)["rules"])
    ...
    s.anchor("synthetic:finding-independent-review-anchor:evidence",docket,fset,stage_digest,sha256(manifest_bytes).hexdigest(),lessons,rules,submissions,rereviews,findings,roles,sc,sv,rc,rv,fc,fv,25,True)
    for i,(flow,kind) in enumerate(CASE_KEYS):
    ...
    e.update({"lesson_registry_digest":stage_digest,"remediation_manifest_digest":sha256(manifest_bytes).hexdigest(),"audit_remediation":"ETH-11101-AUDIT-001"})
