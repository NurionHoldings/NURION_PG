"""Generate deterministic evidence for patch operator decision intake."""
...
def main() -> None:
    ...
    if (
        ...
        or policy["required_packet_scope"] != PATCH_PACKET_SCOPE
        or tuple(policy["allowed_decisions"])
        != PATCH_ALLOWED_OPERATOR_DECISIONS
        or tuple(policy["required_acknowledgements"])
        != PATCH_REQUIRED_ACKNOWLEDGEMENTS
        or policy["maximum_state"] != "SYNTHETIC_PATCH_DECISION_VALIDATED"
        or policy["synthetic_verification_only"] is not True
        or any(policy[field] is not False for field in forbidden)
    ):
        raise SystemExit("patch operator decision intake policy drift detected")

    findings = sha256(b"patch operator decision intake evidence").hexdigest()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        patch_review_report = reopened_patch_review.evidence()
        packet_source_before = reopened_patch_review.evidence()["report_digest"]
        patch_packet_book = SyntheticPatchOperatorDecisionPacketBook()
        patch_packet = patch_packet_book.prepare(
            reopened_patch_review,
            patch_review_record.shadow_id,
            governance,
            generated_at=NOW,
        )
        packet_source_after = reopened_patch_review.evidence()["report_digest"]
        patch_packet_report = patch_packet_book.evidence()
        patch_decision = (
            SyntheticPatchOperatorDecision.AUTHORIZE_SYNTHETIC_PATCH_DRAFT
        )
        patch_signing_value = {
            "envelope_id": (
                "synthetic:patch-operator-decision-envelope:evidence"
            ),
            "packet_id": patch_packet.packet_id,
            "packet_digest": patch_packet.packet_digest,
            "operator_id": SYNTHETIC_OPERATOR_ID,
            "decision": patch_decision.value,
            "requested_scope": PATCH_PACKET_SCOPE,
            "acknowledgements": list(PATCH_REQUIRED_ACKNOWLEDGEMENTS),
            "issued_at": NOW.isoformat(),
            "expires_at": (NOW + DECISION_ENVELOPE_VALIDITY).isoformat(),
            "nonce": "synthetic:patch-operator-nonce:evidence",
            "key_id": KEY_ID,
        }
        patch_signature = "sha256=" + hmac.new(
            SECRET.encode("utf-8"),
            json.dumps(
                patch_signing_value,
                ensure_ascii=False,
        ...
        patch_envelope = SyntheticPatchOperatorDecisionEnvelope(
            patch_signing_value["envelope_id"],
            patch_packet.packet_id,
            patch_packet.packet_digest,
            SYNTHETIC_OPERATOR_ID,
            patch_decision,
            PATCH_PACKET_SCOPE,
            PATCH_REQUIRED_ACKNOWLEDGEMENTS,
            NOW,
            NOW + DECISION_ENVELOPE_VALIDITY,
            patch_signing_value["nonce"],
            KEY_ID,
            patch_signature,
        )
        patch_intake = SyntheticPatchOperatorDecisionIntake(registry)
        patch_packet_before = patch_packet_book.evidence()["report_digest"]
        patch_intent_assessment = patch_intake.assess(
            patch_packet_book,
            patch_review_record.shadow_id,
            patch_envelope,
            received_at=NOW,
        )
        patch_packet_after = patch_packet_book.evidence()["report_digest"]
        report = patch_intake.evidence()
    ...
    if (
        ...
        or first_patch_review_report != patch_review_report
        or patch_review_report["audit_chain_valid"] is not True
        or patch_review_report["record_bindings_valid"] is not True
        or packet_source_before != packet_source_after
        or patch_packet.shadow_id != patch_assessment.shadow_id
        or patch_packet.proposal_id != proposal.proposal_id
        or patch_packet.proposal_digest != proposal.proposal_digest
        or patch_packet.implementation_review_digest
        != patch_assessment.source_review_digest
        or patch_packet.shadow_assessment_digest
        != patch_assessment.assessment_digest
        or patch_packet.shadow_review_digest
        != ready_patch_source.review_digest
        or patch_packet.requested_scope != PATCH_PACKET_SCOPE
        or patch_packet.allowed_decisions
        != PATCH_ALLOWED_OPERATOR_DECISIONS
        or patch_packet.required_acknowledgements
        != PATCH_REQUIRED_ACKNOWLEDGEMENTS
        or patch_packet.operator_decision_recorded
        or patch_packet.patch_content_present
        or patch_packet.code_change_allowed
        or patch_packet.automatic_application_allowed
        or patch_packet.safety_baseline_relaxation_allowed
        or patch_packet.execution_allowed
        or patch_packet.production_activation_allowed
        or patch_packet_report["packet_chain_valid"] is not True
        or patch_packet_report["maximum_state"] != "AWAITING_OPERATOR_DECISION"
        or patch_packet_before != patch_packet_after
        or patch_intent_assessment.packet_id != patch_packet.packet_id
        or patch_intent_assessment.packet_digest != patch_packet.packet_digest
        or patch_intent_assessment.decision is not patch_decision
        or patch_intent_assessment.validation_state
        != "SYNTHETIC_PATCH_DECISION_VALIDATED"
        or patch_intent_assessment.packet_state_changed
        or patch_intent_assessment.operator_decision_recorded
        or patch_intent_assessment.patch_content_present
        or patch_intent_assessment.code_change_allowed
        or patch_intent_assessment.automatic_application_allowed
        or patch_intent_assessment.safety_baseline_relaxation_allowed
        or patch_intent_assessment.execution_allowed
        or patch_intent_assessment.production_activation_allowed
        or report["assessment_chain_valid"] is not True
        or report["maximum_state"] != "SYNTHETIC_PATCH_DECISION_VALIDATED"
        ...
        raise SystemExit("patch operator decision intake evidence boundary failed")

    output = ROOT / "build/synthetic-patch-operator-intake-evidence.json"
