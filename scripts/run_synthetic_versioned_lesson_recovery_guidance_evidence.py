...
def recovery_service(live,rules,manifest_digest):
    prior_lessons,prior_registry_digest=validated_stage_snapshot(live,PRIOR_LESSONS,"ARKAON-LESSONS-12301",(9901,12700));lessons,registry_digest=validated_stage_snapshot(live,APPLIED_LESSONS,"ARKAON-LESSONS-12701",(12701,13100))
    ...
    s.finalize("synthetic:recovery-guidance-docket:evidence","synthetic:recovery-docket-compiler:final-compiler","synthetic:recovery-docket-validator:final-validator");return s,registry_digest
def main():
    rb=REGISTRY.read_bytes();mb=MANIFEST.read_bytes();manifest_digest=sha256(mb).hexdigest();rules=tuple(x["id"] for x in json.loads(POLICY.read_bytes())["rules"]);live=validate(json.loads(rb),declared_remediations(json.loads(mb)),discovered_negative_tests());s,registry_digest=recovery_service(live,rules,manifest_digest);e=s.evidence()
