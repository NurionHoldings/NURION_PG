...
def main():
    ...
    try:
        ...
        admin.commit()
        case_id="rq_case_18301";case_digest=sha256(b"exact-quarantine-case-envelope").hexdigest();admin.execute(f"INSERT INTO {q}(case_id,case_digest,case_state) VALUES(%s,%s,'QUARANTINED_NON_EXECUTABLE_ONLY')",(case_id,case_digest));admin.commit()
        ...
        wrong_digest=dict(r,disposition_key="independent-wrong-digest-key",decision_sequence=3,case_digest=sha256(b"wrong-case-digest").hexdigest());wrong_digest["disposition_id"]=deterministic_disposition_id(case_id,wrong_digest["disposition_key"],wrong_digest["case_digest"],reviewer,DISPOSITION_STATE);wrong_digest_failed=rejected(admin,lambda:exact_insert_or_read(admin,t,wrong_digest))
        ...
        if not exact or count!=1 or q_digest!=case_digest or not all((changed_failed,cross_failed,wrong_digest_failed,update_failed,delete_failed,view_insert,view_update,view_delete,quarantine_update)):raise RuntimeError("recovery disposition invariant failed; proof HOLD")
