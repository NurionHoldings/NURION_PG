...
def main():
    ...
    live=validate(json.loads(registry_bytes),declared_remediations(json.loads(manifest_bytes)),discovered_negative_tests());lessons,stage_digest=validated_stage_snapshot(live,APPLIED_LESSONS,"ARKAON-LESSONS-12301",(9901,12700));rules=tuple(x["id"] for x in json.loads(guidance_bytes)["rules"])
    ...
    service.anchor("synthetic:finding-observation-anchor:evidence",docket,rset,stage_digest,sha256(manifest_bytes).hexdigest(),lessons,rules,submissions,rereviews,actors,sc,sv,rc,rv,24,True)
    for position,(flow,kind) in enumerate(CASE_KEYS):
    ...
    evidence.update({"lesson_registry_digest":stage_digest,"remediation_manifest_digest":sha256(manifest_bytes).hexdigest(),"applied_lesson_ids":lessons,"applied_rule_ids":rules,"audit_remediation":"ETH-10701-AUDIT-001"})
