...
def option_service(lessons,rules,registry_digest,manifest_digest):
    prior=prior_service(lessons,rules,registry_digest,manifest_digest);stress=SyntheticJudgmentPreparationStressTest()
    ...
    stress.anchor(prior,registry_digest,manifest_digest,lessons,rules,26,True)
    ...
    s.anchor(stress,registry_digest,manifest_digest,lessons,rules,27,True)
    for i,key in enumerate(CASE_KEYS):
    ...
    s.finalize("synthetic:option-methodology-docket:evidence","synthetic:methodology-docket-compiler:methodology-final-compiler","synthetic:methodology-docket-validator:methodology-final-validator")
    return s
def main():
    rb=REGISTRY.read_bytes();mb=MANIFEST.read_bytes();rules=tuple(x["id"] for x in json.loads(GUIDANCE.read_bytes())["rules"]);live=validate(json.loads(rb),declared_remediations(json.loads(mb)),discovered_negative_tests());lessons,stage_digest=validated_stage_snapshot(live,APPLIED_LESSONS,"ARKAON-LESSONS-12301",(9901,12700))
    if lessons!=APPLIED_LESSONS or rules!=APPLIED_RULES:raise ValueError("continuous lessons and rules required")
    s=option_service(lessons,rules,stage_digest,sha256(mb).hexdigest());e=s.evidence()
