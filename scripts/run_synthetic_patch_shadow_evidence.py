"""Generate deterministic evidence for metadata-only synthetic patch shadow."""
...
def main() -> None:
    policy = read_json("config/synthetic-patch-shadow-policy.json")
    forbidden = (
    ...
    if (
        ...
        or policy["required_input_state"] != MAXIMUM_REVIEW_STATE
        or policy["maximum_decision"] != MAXIMUM_PATCH_SHADOW_DECISION
        or policy["exact_fixture_per_scope_required"] is not True
        or any(policy[field] is not False for field in forbidden)
    ):
        raise SystemExit("synthetic patch shadow policy drift detected")

    findings = sha256(b"synthetic patch shadow evidence").hexdigest()
    ...
    if (
        ...
        or report["maximum_decision"] != MAXIMUM_PATCH_SHADOW_DECISION
        or report["patch_content_present"] is not False
        or report["file_write_method_present"] is not False
        ...
        raise SystemExit("synthetic patch shadow evidence boundary failed")

    output = ROOT / "build/synthetic-patch-shadow-evidence.json"
    output.parent.mkdir(exist_ok=True)
    payload = json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2) + "\n"
    output.write_text(payload, encoding="utf-8")
    digest = sha256(payload.encode("utf-8")).hexdigest()
    output.with_suffix(".json.sha256").write_text(digest + "\n", encoding="ascii")
    print(f"synthetic patch shadow: PASS {digest}")
