"""Generate deterministic evidence for synthetic implementation proposal drafts."""
...
def main() -> None:
    policy = read_json("config/synthetic-implementation-proposal-policy.json")
    forbidden = (
    ...
    if (
        ...
        or policy["required_input_state"] != RECEIPT_STATE
        or policy["required_decision"]
        != "AUTHORIZE_SYNTHETIC_IMPLEMENTATION_PROPOSAL_DRAFT"
        or policy["maximum_state"] != IMPLEMENTATION_PROPOSAL_STATE
        or policy["eternian_review_required"] is not True
        or any(policy[field] is not False for field in forbidden)
    ):
        raise SystemExit("synthetic implementation proposal policy drift detected")

    findings = sha256(b"synthetic implementation proposal evidence").hexdigest()
    ...
    if (
        ...
        or report["proposal_chain_valid"] is not True
        or report["eternian_review_required"] is not True
        or report["maximum_state"] != IMPLEMENTATION_PROPOSAL_STATE
        ...
        raise SystemExit("synthetic implementation proposal evidence boundary failed")

    output = ROOT / "build/synthetic-implementation-proposal-evidence.json"
    output.parent.mkdir(exist_ok=True)
    payload = json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2) + "\n"
    output.write_text(payload, encoding="utf-8")
    digest = sha256(payload.encode("utf-8")).hexdigest()
    output.with_suffix(".json.sha256").write_text(digest + "\n", encoding="ascii")
    print(f"synthetic implementation proposal: PASS {digest}")
